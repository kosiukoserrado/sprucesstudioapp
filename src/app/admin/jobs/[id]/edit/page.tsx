...
import { fetchJobByIdForEdit, updateJob } from "@/lib/firebase/firestore";
...
export default function EditJobPage() {
  ...
  useEffect(() => {
    ...
    const getJob = async () => {
      setFetching(true);
      try {
        const jobData = await fetchJobByIdForEdit(id);
        if (jobData) {
          form.reset({
            ...jobData,
            totalPay: jobData.payment, // Correct mapping
          });
        } else {
        ...
        toast({ variant: "destructive", title: "Error fetching job" });
      } finally {
        setFetching(false);
      }
    };
    ...
    <div className="space-y-8">
       <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-2">
                <h1 className="font-headline text-2xl md:text-3xl font-bold tracking-tight">Edit Job</h1>
